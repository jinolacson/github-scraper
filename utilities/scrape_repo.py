...
def get_top_repos(
    ...
    try:
        ...
        if not Path(full_url_save_path).exists():
            get_top_repo_urls(keyword=keyword, sort_by=sort_by,
                              save_directory=save_directory, start_page=start_page, stop_page=stop_page)
...
def remove_duplicates_in_csv(keyword: int,
                         sort_by: str = 'best_match',
                         start_page: int = 1,
                         stop_page: int = 10,
                         save_directory: str = "."):
    ...
    user_save_path = f"{save_directory}/top_contributor_info_{keyword}_{sort_by}_{start_page}_{stop_page}.csv"
    clean_csv = f"{save_directory}/top_contributor_info_cleaned{keyword}_{sort_by}_{start_page}_{stop_page}.csv"
    ...
    data = pd.read_csv(user_save_path, usecols=usecols)
    new_df=pd.concat([pd.Series(data[i].unique()) for i in data.columns], axis=1)
    new_df.columns=data.columns
    new_df.to_csv(clean_csv)
